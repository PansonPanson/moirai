在程序启动过程中，moirai 会检查已创建的线程池对象，判断其是否为 DynamicThreadPoolExecutor 类型，并检测对象上是否存在 @DynamicThreadPool 注解。如果同时满足这两个条件，则程序将该对象认定为动态线程池，并将其配置信息上报至服务端进行注册。
...
/**
 * @方法描述：
 * 这个就是动态线程池对象的处理器，具体逻辑是这样的，大家在创建动态线程池对象的时候，可以看看DynamicThreadPoolConfig类中给出的几个现成例子
